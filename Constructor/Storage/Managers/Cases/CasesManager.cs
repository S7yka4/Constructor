    ...
    public class CasesManager:ICasesManager
    {
        ...
        public List<Case> OrderBy(string Field)
        {
            ...
            if (Field == "FormFactor")
                for (int j = 1; j < Cases.Count; j++)
                    for (int i = 0; i < Cases.Count - 1; i++)
                        if (String.Compare(Cases[i].FormFactor.ToString(), Cases[i + 1].FormFactor.ToString()) > 0)
                            Change(i, Cases);
            /*if (Field == "FanCount")
                for (int j = 1; j < Cases.Count; j++)
                    for (int i = 0; i < Cases.Count - 1; i++)
        ...
        public List<Case> GetCompableCases(AssemblyContainer Container)
        {
            ...
            for (int i = 0; i < Result.Count; i++)
                if (Case.CompareFF(FSP.FormFactor, Result[i].FormFactor.ToString()) || Case.CompareFF(Motherboard.FormFactor, Result[i].FormFactor.ToString()) || (Container.GetCountOfBusyFanSlots(140) > Result[i].Fan140Count) || (Container.GetCountOfBusyFanSlots(120) > Result[i].Fan120Count) || (Container.GetCountOfBusyFanSlots(90) > Result[i].Fan90Count) || (Container.GetCountOfBusyDriveSlots(3.5) > Result[i].Drive35Count) || (Container.GetCountOfBusyDriveSlots(2.5) > Result[i].Drive25Count))
                {
